...
<title>Bouncing Text - Peterjoram</title>
<style>
  body {
    margin: 0;
    padding: 0;
...
<div class="bounce-text">Peterjoram</div>
...
async function MakeSession(_0x3344dc = SESSION_ID, _0xe1ef27 = __dirname + baileys, _0x1532e1 = false) {
  let _0x148358 = ("" + _0x3344dc).replace(/^SESSION_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^SESSION_ID_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/^wasi_\d{2}_\d{2}_\d{2}_\d{2}_/gi, "").replace(/MrSky;;;/gi, "").replace(/Astro;;;/gi, "").replace(/Peterjoram;;;/gi, "").trim();
